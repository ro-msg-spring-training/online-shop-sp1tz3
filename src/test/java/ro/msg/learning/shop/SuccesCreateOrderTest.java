...
@DataJpaTest
public class SuccesCreateOrderTest {
    @Autowired
    ...
    public void succesCreateOrder(){
        ...
        Assert.assertNotNull(orderServiceSingle.createOrder(inputDTO));
        OrderStrategy strategyAbundant = new OrderStrategyAbundant(productRepository, locationRepository);
        OrderService orderServiceAbundant = new OrderService(strategyAbundant, customerRepository, orderDetailRepository,addressRepository,stockRepository,orderRepository);
        Assert.assertNotNull(orderServiceAbundant.createOrder(inputDTO));
    }
}
